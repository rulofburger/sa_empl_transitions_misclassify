...
We allow transition probabilities to depend on individual characteristics
...
and is excluded from the job-entry equation:
\[
  \theta_{0i} = \Phi(X_{0i} \beta_0), \qquad
  \theta_{1i} = \Phi(X_{1i} \beta_1),
\]
where $X_{1i}$ additionally contains the contract-type indicators and
$\Phi(\cdot)$ is the standard normal CDF. The misclassification parameter
