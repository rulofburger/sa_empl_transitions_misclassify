...
Let $h_{t} \in \{0,1\}$ denote the true employment status at
quarter $t$, where $h_{t} = 1$ indicates employment. We assume that the true
process follows a first-order, time-homogeneous Markov chain with transition
probabilities
\begin{equation}
  \theta_0 \equiv \Pr(h_{t}=1 \mid h_{t-1}=0), \qquad
  \theta_1 \equiv \Pr(h_{t}=0 \mid h_{t-1}=1),
  \label{eq:baseline_transition}
\end{equation}
The stationary (long-run) employment rate is $\alpha = \theta_0 / (\theta_0 + \theta_1)$.
...
Survey respondents report $s_{t} \in \{0,1\}$, their observed employment
status, which may differ from $h_{t}$ due to measurement error. We assume
a symmetric, time-invariant misclassification structure:
\begin{equation}
  \pi \equiv \Pr(s_{t} = 1 \mid h_{t} = 0) = \Pr(s_{t} = 0 \mid h_{t} = 1),
  \label{eq:baseline_error}
\end{equation}
...
Standard errors are obtained by a parametric bootstrap with $B=200$
repetitions, sampling from the multinomial distribution over the eight
observed histories at the estimated cell probabilities.
...
We allow the entry and exit probabilities to depend on individual characteristics
$X_i$ (age, education, gender, race, contract type) through probit link functions:
\[
  \theta_{0i} = \Phi(X_i \beta_0), \qquad
  \theta_{1i} = \Phi(X_i \beta_1),
\]
where $\Phi(\cdot)$ is the standard normal CDF. The misclassification parameter
...
algorithm of \citet{dempster1977}, performing a single gradient step in the
direction of the expected complete-data likelihood gradient at each iteration.
The GEM algorithm is guaranteed to produce non-decreasing likelihoods and
converges under the same conditions as the full EM.
