...
\[
  ...
  \theta_1 = \Pr(h_{t}=1 \mid h_{t-1}=1),
\]
where we suppress the individual subscript for clarity. The stationary
(long-run) employment rate is
$\alpha = \theta_0 / (\theta_0 + 1 - \theta_1)$; the employment exit rate is
$1-\theta_1$.
...
\begin{align}
  ...
  \theta_1^{(k+1)} &= \frac{\sum_i \sum_{\mathbf{h}: h_1=1, h_2=1} \gamma_{i\mathbf{h}}^{(k)}
                            + \sum_i \sum_{\mathbf{h}: h_2=1, h_3=1} \gamma_{i\mathbf{h}}^{(k)}}
...
$\theta_1$ is the analogous employment-persistence fraction (so
$1-\theta_1$ is the exit fraction), and $\pi$ is the
responsibility-weighted empirical misclassification rate. With a freely
estimated initial probability these updates define an exact EM algorithm.
Under stationarity, however, the initial-state term couples $\theta_0$ and
$\theta_1$, so imposing stationarity only after the closed-form transition
updates need not maximise the constrained likelihood.

For the reported estimates we therefore maximise the observed-data likelihood
directly after collapsing the sample to the eight possible observed histories.
This collapse is exact because individuals with the same three-wave history
have the same likelihood contribution. Probabilities are parameterised on
unconstrained logit scales, the stationary initial probability is imposed inside
the likelihood, and several starts are used. The symmetric model is initialized
from the corresponding no-error solution and the asymmetric model from the
symmetric solution. We require a small likelihood score, positive observed
information, and the likelihood inequalities implied by nesting before retaining
a result. The EM estimates remain useful as starting values and for calculating
posterior latent-history probabilities.

For the baseline and covariate specifications, the reported standard errors use an
individual-level survey-weighted sandwich covariance matrix. We obtain the
observed-information ``bread'' from the observed likelihood and use the outer
product of weighted individual scores as the ``meat.''
...
Employment-status misclassification may arise either because a respondent's
status is recorded incorrectly or because observations belonging to different
people are incorrectly linked across waves. Both mechanisms should be associated
with inconsistencies in other characteristics of the linked record. We therefore
allow $\pi$ to depend on wave-attributed indicators of record unreliability:
\begin{itemize}
  \item $A_{it}=1$ if an age inconsistency is attributed to wave $t$;
  \item $E_{it}=1$ if an education inconsistency is attributed to wave $t$.
\end{itemize}
We then estimate the generalised model:
\[
  \pi_{it} = \tfrac{1}{2}\mathrm{logit}^{-1}
  (\delta_0 + \delta_1 A_{it} + \delta_2 E_{it}),
\]
where the factor one-half imposes $\pi_{it}<0.5$. Pairwise discrepancies are
attributed to the first, middle, or final wave using the rule described in
Appendix~\ref{app:baseline}. The main specification estimates the initial
employment probability freely and reports the survey-weighted mean of
$\pi_{it}$. As robustness checks, we allow the true transition process to differ
between records with and without any inconsistency and distinguish mild from
