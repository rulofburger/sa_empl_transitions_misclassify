...
.covariate_q <- function(par, suff, X, entry_active, stationary,
                         persistence_active = rep(TRUE, ncol(.as_transition_design(X)$X12)),
                         gradient = FALSE) {
  X_transition <- .as_transition_design(X)
  p <- ncol(X_transition$X12)
  p0 <- sum(entry_active)
  p1 <- sum(persistence_active)
  beta0 <- numeric(p)
  beta0[entry_active] <- par[seq_len(p0)]
  beta1 <- numeric(p)
  beta1[persistence_active] <- par[p0 + seq_len(p1)]
  ...
  for (nm in c("X12", "X23")) {
    ...
    grad1 <- grad1 + as.vector(crossprod(Xt[, persistence_active, drop = FALSE], score_eta1))
  ...
  if (stationary) {
    ...
    grad1 <- grad1 + as.vector(crossprod(
      X_transition$X12[, persistence_active, drop = FALSE], score_eta1_init))
  }
...
.interpolate_cov_params <- function(old, proposed, fraction, entry_active,
                                    persistence_active = rep(TRUE, length(old$beta1))) {
  out <- old
  ...
  out$beta1[!persistence_active] <- 0
...
m_step_covariates <- function(suff, X, params_old, model_type = "symmetric",
                               ...
                               persistence_active = attr(X, "persistence_active")) {
  ...
  if (!is.logical(persistence_active) || length(persistence_active) != p || !any(persistence_active))
    stop("m_step_covariates: persistence_active must be a logical vector of length ncol(X)")

  old <- params_old
  old$beta0[!entry_active] <- 0
  old$beta1[!persistence_active] <- 0
  par_old <- c(old$beta0[entry_active], old$beta1[persistence_active])

  opt <- optim(
    par = par_old,
    fn = function(z) -.covariate_q(z, suff, X, entry_active, stationary, persistence_active),
    gr = function(z) -.covariate_q(z, suff, X, entry_active, stationary, persistence_active,
                                    gradient = TRUE),
    method = "BFGS",
  ...
  proposed$beta1[persistence_active] <- opt$par[p0 + seq_len(p1)]
  proposed$beta1[!persistence_active] <- 0
  ...
  q_old <- .covariate_q(par_old, suff, X, entry_active, stationary, persistence_active)
  par_new <- c(proposed$beta0[entry_active], proposed$beta1[persistence_active])
  q_new <- .covariate_q(par_new, suff, X, entry_active, stationary, persistence_active)
