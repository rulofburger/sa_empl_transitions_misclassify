...
local({
  ...
  bootstrap <- function(version, library) {
    ...
    # add empty line to break up bootstrapping from normal outpu
    catf("")
    return(invisible())
  }
  ...
  renv_bootstrap_repos <- function() {
    ...
    if (!is.na(repos)) {
      ...
      # split into named repositories if presen
    ...
    # add in renv.bootstrap.repos if se
  ...
  renv_bootstrap_download_cran_latest_find <- function(version) {
    ...
    for (type in types) {
      for (repos in renv_bootstrap_repos()) {
        ...
        # add custom headers if available -- note tha
  ...
  renv_bootstrap_download_tarball <- function(version) {
    ...
    # an environment variable, then use i
    ...
    # bail if it doesn't exis
    if (!file.exists(tarball)) {

      # let the user know we weren't able to honour their reques
  ...
  renv_bootstrap_install_impl <- function(library, tarball) {
    ...
    # invoke using system2 so we can capture and report outpu
    bin <- R.home("bin")
  ...
  renv_bootstrap_platform_prefix_impl <- function() {
    ...
    # if an explicit prefix has been supplied, use i
    prefix <- Sys.getenv("RENV_PATHS_PREFIX", unset = NA)
    if (!is.na(prefix))
      return(prefix)

    # if the user has requested an automatic prefix, generate i
  ...
  renv_bootstrap_validate_version <- function(version, description = NULL) {
    ...
    # use the loaded version of the package by default anyhow. note tha
  ...
  renv_bootstrap_profile_load <- function(project) {
    ...
    # if RENV_PROFILE is already set, just use tha
