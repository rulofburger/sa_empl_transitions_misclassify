...
calc_gmm_moments_4waves <- function(param_transformed, df_x) {
  ...
  param_temp <- logit_inverse(param_transformed)
  param <- data.frame(
    theta_0 = param_temp[1],
    theta_01 = param_temp[2],
    theta_10 = param_temp[3],
    theta_1 = param_temp[4],
    pi = param_temp[5]
  )
  ...
  df_probs1 <- df_probs %>% 
    mutate(y = paste0(y1, y2, y3, y4)) %>% 
    select(y, joint_p) %>% 
    pivot_wider(names_from = y, values_from = joint_p, names_prefix = "y") %>% 
    unlist() %>% as.vector()
  
# print(df_probs1)

 g <- sweep(df_x[, 1:16], 2, df_probs1)
# Use weights to calculate weighted probabilities
 g <- g*df_x[, 17]
 g <- g[, 1:15]
...
calc_gmm_derivatives_4waves <- function(param_transformed, df_x) {
  ...
  df_gi <- df_estimate %>% 
    ...
    mutate(
      lgi_theta_0 = weight*joint_d_theta_0,
      lgi_theta_01 = weight*joint_d_theta_01,
      lgi_theta_10 = weight*joint_d_theta_10,
      lgi_theta_1 = weight*joint_d_theta_1,
      lgi_pi = weight*joint_d_pi
    ...
    group_by(y1, y2, y3, y4) %>% 
    summarise(
      lgi_theta_0 = mean(lgi_theta_0),
      lgi_theta_01 = mean(lgi_theta_01),
      lgi_theta_10 = mean(lgi_theta_10),
      lgi_theta_1 = mean(lgi_theta_1),
      lgi_pi = mean(lgi_pi)
    ) %>% 
    ungroup() %>% 
...
calc_gmm_derivatives_4waves_pi0 <- function(param_transformed, df_x) {
