...
calc_lli_derivatives_3waves_ar1_fmm2 <- function(param_transformed, pi0 = FALSE) {
  ...
  param <- logit_inverse(param_transformed)
  param <- as.list( logit_inverse(param_transformed))
  mu_1 <- param$theta0_1/(param$theta0_1 + param$theta1_1)
  mu_2 <- param$theta0_2/(param$theta0_2 + param$theta1_2)
  if(pi0) param$pi <- 0
...
gmm_moments <- function(theta, data) {
  # ignore `data` here because your calc_lli... reads df_template and df_estimate
  df_gi <- calc_lli_derivatives_3waves_ar1_fmm2(theta, pi0 = FALSE)
  # convert tibble to plain matrix
  as.matrix(df_gi)
}
