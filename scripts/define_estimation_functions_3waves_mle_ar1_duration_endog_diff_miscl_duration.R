...
calc_lli_3waves_ar1_duration_endog <- function(param_transformed, pi_fixed = NULL, only_pi = NULL) {
  ...
  if(!is.null(only_pi)) {
    ...
    param$lambda_g_mis <- exp(only_pi$lambda_g_mis) + offset
    param$lambda_h_mis <- exp(only_pi$lambda_h_mis) + offset
    ...
    param$lambda_g_mis <- exp(param_transformed$lambda_g_mis) + offset
    param$lambda_h_mis <- exp(param_transformed$lambda_h_mis) + offset
  ...
  df_probs_temp <- df_template_duration_endog %>% 
    mutate(
      ...
      p_g1 = case_when(
        y1 == 0 ~ 1,
        y1 == 1 & y1_star == 0 ~ gamlss.dist::dexGAUS(tenure1, mu = 0, sigma = param$sigma_g, nu = param$lambda_g_mis),
        y1 == 1 & y1_star == 1 ~ gamlss.dist::dexGAUS(tenure1, mu = 0, sigma = param$sigma_g, nu = param$lambda_g)
      ),
      p_g2 = case_when(
        y2 == 0 ~ 1,
        y2 == 1 & y2_star == 0 ~ gamlss.dist::dexGAUS(tenure2, mu = 0, sigma = param$sigma_g, nu = param$lambda_g_mis),
      ...
      p_g3 = case_when(
        y3 == 0 ~ 1,
        y3 == 1 & y3_star == 0 ~ gamlss.dist::dexGAUS(tenure3, mu = 0, sigma = param$sigma_g, nu = param$lambda_g_mis),
      ...
      p_h1 = case_when(
        y1 == 1 ~ 1,
        y1 == 0 & y1_star == 0 ~ gamlss.dist::dexGAUS(timegap1, mu = 0, sigma = param$sigma_h, nu = param$lambda_h),
        y1 == 0 & y1_star == 1 ~ gamlss.dist::dexGAUS(timegap1, mu = 0, sigma = param$sigma_h, nu = param$lambda_h_mis)
      ),
      p_h2 = case_when(
        y2 == 1 ~ 1,
        y2 == 0 & y2_star == 1 ~ gamlss.dist::dexGAUS(timegap2, mu = 0, sigma = param$sigma_h, nu = param$lambda_h_mis),
      ...
      p_h3 = case_when(
        y3 == 1 ~ 1,
        y3 == 0 & y3_star == 1 ~ gamlss.dist::dexGAUS(timegap3, mu = 0, sigma = param$sigma_h, nu = param$lambda_h_mis),
...
calc_lli_derivatives_3waves_ar1_duration_endog <- function(param_transformed, pi_fixed = NULL, only_pi = NULL) {
  ...
  df_probs_temp <- df_template_duration_endog %>% 
    mutate(
      ...
      d1_lambda_g = case_when(
        y1 == 0 ~ 0,
        y1 == 1 & y1_star == 0 ~ 0,
        y1 == 1 & y1_star == 1 ~ partial_exGauss_lambda(x = tenure1, sigma = param$sigma_g, lambda = param$lambda_g)
      ),
      d1_lambda_g_mis = case_when(
        y1 == 0 ~ 0,
        y1 == 1 & y1_star == 1 ~ 0,
        y1 == 1 & y1_star == 0 ~ partial_exGauss_lambda(x = tenure1, sigma = param$sigma_g, lambda = param$lambda_g_mis)
      ),
      d1_sigma_g = case_when(
        y1 == 0 ~ 0,
        y1 == 1 & y1_star == 0 ~ partial_exGauss_sigma(x = tenure1, sigma = param$sigma_g, lambda = param$lambda_g_mis),
        y1 == 1 & y1_star == 1 ~ partial_exGauss_sigma(x = tenure1, sigma = param$sigma_g, lambda = param$lambda_g)
      ),
      d1_lambda_h = case_when(
        y1 == 1 ~ 0,
        y1 == 0 & y1_star == 1 ~ 0,
        y1 == 0 & y1_star == 0 ~ partial_exGauss_lambda(x = timegap1, sigma = param$sigma_h, lambda = param$lambda_h)
      ),
      d1_lambda_h_mis = case_when(
        y1 == 1 ~ 0,
        y1 == 0 & y1_star == 0 ~ 0,
        y1 == 0 & y1_star == 1 ~ partial_exGauss_lambda(x = timegap1, sigma = param$sigma_h, lambda = param$lambda_h_mis)
      ),
      d1_sigma_h = case_when(
        y1 == 1 ~ 0,
        y1 == 0 & y1_star == 1 ~ partial_exGauss_sigma(x = timegap1, sigma = param$sigma_h, lambda = param$lambda_h_mis),
        y1 == 0 & y1_star == 0 ~ partial_exGauss_sigma(x = timegap1, sigma = param$sigma_h, lambda = param$lambda_h),
      ),
      ...
      d2_lambda_g_mis = case_when(
        y2 == 0 ~ 0,
        y2 == 1 & y2_star == 0 ~ partial_exGauss_sigma(x = tenure1, sigma = param$sigma_g, lambda = param$lambda_g_mis),
        y2 == 1 & y2_star == 1 & y1_star == 0 ~ 0,
        y2 == 1 & y2_star == 1 & y1_star == 1 & y1 == 1 ~ 0,
        y2 == 1 & y2_star == 1 & y1_star == 1 & y1 == 0 ~ 0
      ),
      d2_sigma_g = case_when(
        y2 == 0 ~ 0,
        y2 == 1 & y2_star == 0 ~ partial_exGauss_sigma(x = tenure2, sigma = param$sigma_g, lambda = param$lambda_g_mis),
      ...
      d2_lambda_h_mis = case_when(
        y2 == 1 ~ 0,
        y2 == 0 & y2_star == 1 ~ partial_exGauss_lambda(x = timegap2, sigma = param$sigma_h, lambda = param$lambda_h_mis),
        y2 == 0 & y2_star == 0 & y1_star == 1 ~ 0,
        y2 == 0 & y2_star == 0 & y1_star == 0 & y1 == 0 ~ 0,
        y2 == 0 & y2_star == 0 & y1_star == 0 & y1 == 1 ~ partial_exGauss_lambda(x = timegap2 - 0.25, sigma = param$sigma_h, lambda = param$lambda_h)
      ),
      d2_sigma_h = case_when(
        y2 == 1 ~ 0,
        y2 == 0 & y2_star == 1 ~ partial_exGauss_sigma(x = timegap2, sigma = param$sigma_h, lambda = param$lambda_h_mis),
      ...
      d3_lambda_g_mis = case_when(
        y3 == 0 ~ 0,
        y3 == 1 & y3_star == 0 ~ partial_exGauss_lambda(x = tenure3, sigma = param$sigma_g, lambda = param$lambda_g_mis),
        y3 == 1 & y3_star == 1 & y2_star == 0 ~ 0,
        y3 == 1 & y3_star == 1 & y2_star == 1 & y2 == 1 ~ 0,
        y3 == 1 & y3_star == 1 & y2_star == 1 & y2 == 0 & y1_star == 0 ~ 0,
        y3 == 1 & y3_star == 1 & y2_star == 1 & y2 == 0 & y1_star == 1 & y1 == 1 ~ 0,
        y3 == 1 & y3_star == 1 & y2_star == 1 & y2 == 0 & y1_star == 1 & y1 == 0 ~ 0
      ),
      d3_sigma_g = case_when(
        y3 == 0 ~ 0,
        y3 == 1 & y3_star == 0 ~ partial_exGauss_sigma(x = tenure3, sigma = param$sigma_g, lambda = param$lambda_g_mis),
      ...
      d3_lambda_h_mis = case_when(
        y3 == 1 ~ 0,
        y3 == 0 & y3_star == 1 ~ partial_exGauss_lambda(x = timegap3, sigma = param$sigma_h, lambda = param$lambda_h_mis),
        y3 == 0 & y3_star == 0 & y2_star == 1 ~ 0,
        y3 == 0 & y3_star == 0 & y2_star == 0 & y2 == 0 ~ 0,
        y3 == 0 & y3_star == 0 & y2_star == 0 & y2 == 1 & y1_star == 1 ~ 0,
        y3 == 0 & y3_star == 0 & y2_star == 0 & y2 == 1 & y1_star == 0 & y1 == 0 ~ 0,
        y3 == 0 & y3_star == 0 & y2_star == 0 & y2 == 1 & y1_star == 0 & y1 == 1 ~ 0
      ),
      d3_sigma_h = case_when(
        y3 == 1 ~ 0,
        y3 == 0 & y3_star == 1 ~ partial_exGauss_sigma(x = timegap3, sigma = param$sigma_h, lambda = param$lambda_h_mis),
      ...
      joint_d_lambda_g_mis = 
        d1_lambda_g_mis*joint_p/p_g1 + 
        d2_lambda_g_mis*joint_p/p_g2 + 
        d3_lambda_g_mis*joint_p/p_g3,
      joint_d_sigma_g = 
      ...
      joint_d_lambda_h_mis = 
        d1_lambda_h_mis*joint_p/p_h1 + 
        d2_lambda_h_mis*joint_p/p_h2 + 
        d3_lambda_h_mis*joint_p/p_h3,
      joint_d_sigma_h = 
  ...
  df_grad <- df_probs_temp %>% 
    ...
    summarise(
      ...
      joint_d_lambda_g_mis = sum(joint_d_lambda_g_mis),
      joint_d_sigma_g = sum(joint_d_sigma_g),
      joint_d_lambda_h = sum(joint_d_lambda_h),
      joint_d_lambda_h_mis = sum(joint_d_lambda_h_mis),
    ...
    mutate(
      ...
      joint_d_pi = joint_d_pi*(param$pi*(1 - param$pi)*2),
      joint_d_lambda_g = joint_d_lambda_g*param$lambda_g,
      joint_d_lambda_g_mis = joint_d_lambda_g_mis*param$lambda_g_mis,
      joint_d_sigma_g = joint_d_sigma_g*param$sigma_g,
      joint_d_lambda_h = joint_d_lambda_h*param$lambda_h,
      joint_d_lambda_h_mis = joint_d_lambda_h_mis*param$lambda_h_mis,
  ...
  df_gi <- df_estimate %>% 
    ...
    mutate(
      ...
      lgi_lambda_g_mis = weight*joint_d_lambda_g_mis/joint_p,
      lgi_sigma_g = weight*joint_d_sigma_g/joint_p,
      lgi_lambda_h = weight*joint_d_lambda_h/joint_p,
      lgi_lambda_h_mis = weight*joint_d_lambda_h_mis/joint_p,
      lgi_sigma_h = weight*joint_d_sigma_h/joint_p,
    ) %>%
    select(lgi_theta_0, lgi_theta_1, lgi_lambda_g, lgi_lambda_h, lgi_lambda_g_mis, lgi_lambda_h_mis, lgi_sigma_g, lgi_sigma_h, lgi_pi)
  
  if(!is.null(pi_fixed)) df_gi <- df_gi %>% select(lgi_theta_0, lgi_theta_1, lgi_lambda_g, lgi_lambda_h, lgi_lambda_g_mis, lgi_lambda_h_mis, lgi_sigma_g, lgi_sigma_h)
