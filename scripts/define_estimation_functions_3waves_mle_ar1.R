...
compute_probs_temp_3waves_ar1_asymmetric <- function(param, mu) {
  ...
  p_obs <- function(y, y_star, pi_0, pi_1) {
    fifelse(y == 0 & y_star == 0, 1 - pi_0,
            fifelse(y == 1 & y_star == 0,     pi_0,
                    fifelse(y == 0 & y_star == 1,     pi_1,
                            1 - pi_1)))
  }
...
calc_lli_3waves_ar1_asymmetric_fst <- function(param_transformed) {
  ...
  df_probs_temp <- 
    compute_probs_temp_3waves_ar1_asymmetric(param, mu)
...
calc_lli_derivatives_3waves_ar1_asymmetric_fst <- function(param_transformed) {
  ...
  df_probs_temp <- df_template |> 
    ...
    p2_star           = p2_star_vals[idx12],
    p3_star           = p3_star_vals[idx23],
    p1                = p_obs(y1, y1_star, param$pi_0, param$pi_1),
    p2                = p_obs(y2, y2_star, param$pi_0, param$pi_1),
    p3                = p_obs(y3, y3_star, param$pi_0, param$pi_1)) |> 
    fmutate(
      joint_p         = p1_star * p1 * p2_star * p2 * p3_star * p3,
      d1_star_theta_0 = fifelse(y1_star == 0,
                                -param$theta_1 / (param$theta_1 + param$theta_0)^2,
      ...
      d1_star_theta_1 = fifelse(y1_star == 0,
                                param$theta_0 / (param$theta_1 + param$theta_0)^2,
                                -param$theta_0 / (param$theta_1 + param$theta_0)^2),
      d2_star_theta_0 = as.numeric(idx12 %in% c(1, 2)) * c(-1, 1)[y2_star + 1],
      d2_star_theta_1 = as.numeric(idx12 %in% c(3, 4)) * c(1, -1)[y2_star - 1],
      d3_star_theta_0 = as.numeric(idx23 %in% c(1, 2)) * c(-1, 1)[y3_star + 1],
      d3_star_theta_1 = as.numeric(idx23 %in% c(3, 4)) * c(1, -1)[y3_star - 1],
      d1_pi_0         = as.numeric(y1 == 0 & y1_star == 0) * -1 + as.numeric(y1 == 1 & y1_star == 0),
      d2_pi_0         = as.numeric(y2 == 0 & y2_star == 0) * -1 + as.numeric(y2 == 1 & y2_star == 0),
      d3_pi_0         = as.numeric(y3 == 0 & y3_star == 0) * -1 + as.numeric(y3 == 1 & y3_star == 0),
      d1_pi_1         = as.numeric(y1 == 0 & y1_star == 1) - as.numeric(y1 == 1 & y1_star == 1),
      d2_pi_1         = as.numeric(y2 == 0 & y2_star == 1) - as.numeric(y2 == 1 & y2_star == 1),
      d3_pi_1         = as.numeric(y3 == 0 & y3_star == 1) - as.numeric(y3 == 1 & y3_star == 1),
      inv_p1_star     = joint_p / p1_star,
      inv_p2_star     = joint_p / p2_star,
      inv_p3_star     = joint_p / p3_star,
      inv_p1          = joint_p / p1,
      ...
      joint_d_theta_0 = d1_star_theta_0 * inv_p1_star + d2_star_theta_0 * inv_p2_star + d3_star_theta_0 * inv_p3_star,
      joint_d_theta_1 = d1_star_theta_1 * inv_p1_star + d2_star_theta_1 * inv_p2_star + d3_star_theta_1 * inv_p3_star,
      joint_d_pi_0    = d1_pi_0 * inv_p1 + d2_pi_0 * inv_p2 + d3_pi_0 * inv_p3,
      joint_d_pi_1    = d1_pi_1 * inv_p1 + d2_pi_1 * inv_p2 + d3_pi_1 * inv_p3)
  ...
  df_grad <- df_probs_temp |>
    ...
    fsummarise(
      joint_d_theta_0 = fsum(joint_d_theta_0),
      joint_d_theta_1 = fsum(joint_d_theta_1),
      joint_d_pi_0    = fsum(joint_d_pi_0),
      joint_d_pi_1    = fsum(joint_d_pi_1),
      joint_p         = fsum(joint_p)) |>
    fungroup() |>
    fmutate(
      joint_d_theta_0 = joint_d_theta_0 * param$theta_0 * (1 - param$theta_0),
