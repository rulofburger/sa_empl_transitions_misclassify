...
calc_df_probs_durdep <- function(df_cov, param_transformed) {
  ...
  df_probs <- df_cov |>
    mutate(
      ...
      p3_star = if_else(y2_star == 1, 
                        if_else(y3_star == 1, 
                                logit_inverse(param_transformed$beta01 + 
                                                param_transformed$beta4A*tenure2),
                                1 - logit_inverse(param_transformed$beta01 + 
                                                    param_transformed$beta4A*tenure2)), 
                        if_else(y3_star == 1, 
                                logit_inverse(param_transformed$beta02 + 
                                                param_transformed$beta4B*timegap2),
                                1 - logit_inverse(param_transformed$beta02 + 
                                                    param_transformed$beta4B*timegap2))),
...
calc_lli_3waves_ar1_durdep <- function(df_template, data, param_transformed) {
  
  # Note: the theta parameters are expanded into linear functions
  #         of the beta parameters to include the covariates
  #         so that:
  #                  1 - theta1 = beta01 + beta4A*tenure
  #                      theta2 = beta02 + beta4B*timegap
  #         where:
  ...
  df_probs <- calc_df_probs_durdep(df_cov            = df_cov, 
...
calc_lli_derivatives_3waves_ar1_durdep <- function(param_transformed, df_x) {
    ...
    mutate(
      d1_star_theta_1 = case_when(
        y1_star == 1 ~ logit_inverse(param_transformed$beta02 + 
                                       param_transformed$beta4B*timegap1) /    # job entry
          ((1 - logit_inverse(param_transformed$beta01 + 
                                param_transformed$beta4A*tenure1) +      # job exit
              logit_inverse(param_transformed$beta02 + 
                              param_transformed$beta4B*timegap1))^2), # job entry
      ...
      joint_d_beta_4A = 
        joint_d_theta_1*(tenure1 + tenure2 + tenure3) +
        joint_d_theta_2*0 ,
  ...
  df_grad <- df_probs_temp |>
    ...
    summarise(
      ...
      joint_d_beta_4A = sum(joint_d_beta_4A), 
      joint_d_beta_4B = sum(joint_d_beta_4B), 
  ...
  df_gi <- df_estimate |>
    ...
    mutate(
      ...
      lgi_beta_4A = weight*lgi_beta_4A/joint_p,
...
calc_mle_3waves_ar1_durdep <- function(param_transformed) {
  ll <- sum(calc_lli_3waves_ar1_durdep(param_transformed))
  return(ll)
}

calc_mle_derivatives_3waves_ar1_durdep <- function(param_transformed) {
  lg <- colSums(calc_lli_derivatives_3waves_ar1_durdep(param_transformed))
  return(lg)
}
