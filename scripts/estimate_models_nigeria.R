...
prob_w4_ar1_pi0 <- df_probs_temp_ar1_pi0 %>% 
  group_by(y1_star, y4_star) %>% 
  summarise(joint_p = sum(joint_p), .groups = "drop") %>% 
  group_by(y1_star) %>%  
  mutate(tot_sum = sum(joint_p)) %>% 
  mutate(cond_p = joint_p/tot_sum) %>% 
  filter(y1_star != y4_star) %>% 
  pull(cond_p)

prob_ever_ar1_pi0 <- df_probs_temp_ar1_pi0 %>% 
  group_by(y1_star, y2_star, y3_star, y4_star) %>%
  summarise(joint_p = sum(joint_p), .groups = "drop") %>% 
  mutate(ever_enter = pmax(y2_star, y3_star, y4_star)) %>% 
  mutate(ever_exit = pmax(1 - y2_star, 1 - y3_star, 1 - y4_star)) %>% 
  group_by(y1_star, ever_enter, ever_exit) %>% 
  summarise(joint_p = sum(joint_p), .groups = "drop") %>% 
  group_by(y1_star) %>% 
  mutate(tot_sum = sum(joint_p)) %>% 
  mutate(cond_p = joint_p/tot_sum) %>% 
  group_by(y1_star, ever_enter) %>% 
  mutate(p_ever_enter = sum(cond_p)) %>% 
  ungroup() %>% 
  group_by(y1_star, ever_exit) %>% 
  mutate(p_ever_exit = sum(cond_p)) %>% 
  filter((y1_star == 0 & ever_enter == 1 & ever_exit == 0) | (y1_star == 1 & ever_enter == 0 & ever_exit == 1)) %>% 
  mutate(cond_p = if_else(y1_star == 0, p_ever_enter, p_ever_exit)) %>%  
  pull(cond_p)
...
prob_ever_ar1 <- df_probs_temp_ar1 %>% 
  group_by(y1_star, y2_star, y3_star, y4_star) %>%
  summarise(joint_p = sum(joint_p), .groups = "drop") %>% 
  mutate(ever_enter = pmax(y2_star, y3_star, y4_star)) %>% 
  mutate(ever_exit = pmax(1 - y2_star, 1 - y3_star, 1 - y4_star)) %>% 
...
prob_w4_ar2_pi0 <- df_probs_temp_ar2_pi0 %>% 
  group_by(y1_star, y4_star) %>% 
  summarise(joint_p = sum(joint_p), .groups = "drop") %>% 
  group_by(y1_star) %>%  
  mutate(tot_sum = sum(joint_p)) %>% 
...
prob_ever_ar2_pi0 <- df_probs_temp_ar2_pi0 %>% 
  group_by(y1_star, y2_star, y3_star, y4_star) %>%
  summarise(joint_p = sum(joint_p), .groups = "drop") %>% 
  mutate(ever_enter = pmax(y2_star, y3_star, y4_star)) %>% 
  mutate(ever_exit = pmax(1 - y2_star, 1 - y3_star, 1 - y4_star)) %>% 
...
prob_ever_ar2 <- df_probs_temp_ar2 %>% 
  group_by(y1_star, y2_star, y3_star, y4_star) %>%
  summarise(joint_p = sum(joint_p), .groups = "drop") %>% 
  mutate(ever_enter = pmax(y2_star, y3_star, y4_star)) %>% 
  mutate(ever_exit = pmax(1 - y2_star, 1 - y3_star, 1 - y4_star)) %>% 
...
df_estimate_probs_4w %>% 
  ...
  write_csv('output/tables/df_estimate_probs_4w.csv')
...
coefficients_3w_ar1_pi0 <- unlist(logit_inverse(model_mle_3w_ar1_pi0$estimate))
std_errors_3w_ar1_pi0 <- unlist(model_mle_3w_ar1_pi0_se)
...
lm_model_3w_ar2_pi0 <- lm(data = df_estimate, y4 ~ y1 + y2 + y3 + y1:y3)
...
stargazer::stargazer(
  lm_model_3w_ar1_pi0, lm_model_3w_ar1, lm_model_3w_ar2_pi0, 
  type = "text", 
  order = c("theta_1", "theta_10", "theta_0", "theta_01", "pi"),
  se = list(std_errors_3w_ar1_pi0, std_errors_3w_ar1, std_errors_3w_ar2_pi0), 
  keep.stat = c("n"), 
  # column.labels = c("AR(1), no ME", "AR(1), ME", "AR(2), no ME"),
  ...
  add.lines = list(
    c("Model", "AR(1)", "AR(1)", "AR(2)"), 
    c("Misclassification", "No", "Yes", "No"),
    c("LL", round(ll_3w_ar1_pi0), round(ll_3w_ar1), round(ll_3w_ar2_pi0))
    )
  )
