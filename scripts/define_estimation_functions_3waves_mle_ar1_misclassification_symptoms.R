...
# FUNCTIONS FOR AR(1) ML ESTIMATOR OVER 3 WAVES WITH COVARIATE DEPENDENT MISCLASSIFICATION ERROR (ONLY AGE) ====

calc_lli_3waves_ar1_misclassifiction_symptoms_age <- function(param_transformed) {
  ...
  df_probs_temp <- df_template_covariates %>% 
    mutate(
      pi = logit_inverse(param$intercept_pi + param$age_pi*age_inconsistent),
  ...
  df_probs <- df_probs_temp %>% 
    group_by(y1, y2, y3, age_inconsistent) %>% 
    summarise(joint_p = sum(joint_p), .groups = "drop")
  
  df_lli <- df_estimate %>% 
    left_join(df_probs, by = c('y1', 'y2', 'y3', 'age_inconsistent')) %>% 
    mutate(lli = weight*log(joint_p)) %>%
    pull(lli)
  
}

calc_lli_derivatives_3waves_ar1_misclassifiction_symptoms_age <- function(param_transformed) {
  ...
  df_gi <- df_estimate %>% 
    left_join(df_grad, by = c('y1', 'y2', 'y3', 'age_inconsistent')) %>% 
    mutate(
    ...
    select(lgi_theta_0, lgi_theta_1, lgi_intercept_pi, lgi_age_pi)
}

calc_mle_3waves_ar1_misclassifiction_symptoms_age <- function(param_transformed) {
  sum(calc_lli_3waves_ar1_misclassifiction_symptoms_age(param_transformed))
}

calc_mle_derivatives_3waves_ar1_misclassifiction_symptoms_age <- function(param_transformed) {
  colSums(calc_lli_derivatives_3waves_ar1_misclassifiction_symptoms_age(param_transformed))
}



# FUNCTIONS FOR AR(1) ML ESTIMATOR OVER 3 WAVES WITH COVARIATE DEPENDENT MISCLASSIFICATION ERROR (ONLY EDUC) ====

calc_lli_3waves_ar1_misclassifiction_symptoms_educ <- function(param_transformed) {
  ...
  df_probs_temp <- df_template_covariates %>% 
    mutate(
      pi = logit_inverse(param$intercept_pi + param$educ_pi*educ_inconsistent),
      mu = param$theta_0/(param$theta_0 + param$theta_1)
    ) %>% 
    mutate(
      p1_star = if_else(y1_star == 1, mu, 1 - mu),
  ...
  df_probs <- df_probs_temp %>% 
    group_by(y1, y2, y3, educ_inconsistent) %>% 
    summarise(joint_p = sum(joint_p), .groups = "drop")
  
  df_lli <- df_estimate %>% 
    left_join(df_probs, by = c('y1', 'y2', 'y3', 'educ_inconsistent')) %>% 
    mutate(lli = weight*log(joint_p)) %>%
    pull(lli)
  
}

calc_lli_derivatives_3waves_ar1_misclassifiction_symptoms_educ <- function(param_transformed) {
  ...
  df_gi <- df_estimate %>% 
    left_join(df_grad, by = c('y1', 'y2', 'y3', 'educ_inconsistent')) %>% 
    mutate(
      lgi_theta_0 = weight*joint_d_theta_0/joint_p,
      lgi_theta_1 = weight*joint_d_theta_1/joint_p,
      lgi_intercept_pi = weight*joint_d_intercept_pi/joint_p,
    ...
    select(lgi_theta_0, lgi_theta_1, lgi_intercept_pi, lgi_educ_pi)
}

calc_mle_3waves_ar1_misclassifiction_symptoms_educ <- function(param_transformed) {
  sum(calc_lli_3waves_ar1_misclassifiction_symptoms_educ(param_transformed))
}

calc_mle_derivatives_3waves_ar1_misclassifiction_symptoms_educ <- function(param_transformed) {
  colSums(calc_lli_derivatives_3waves_ar1_misclassifiction_symptoms_educ(param_transformed))
}



# FUNCTIONS FOR AR(1) ML ESTIMATOR OVER 3 WAVES WITH COVARIATE DEPENDENT MISCLASSIFICATION ERROR (AGE + EDUC) ====
...
# FUNCTIONS FOR AR(1) ML ESTIMATOR OVER 3 WAVES WITH COVARIATE DEPENDENT MISCLASSIFICATION ERROR (AGE + EDUC + AGE:EDUC) ====

calc_lli_3waves_ar1_misclassifiction_symptoms_age_educ <- function(param_transformed) {
  ...
  df_probs_temp <- df_template_covariates %>% 
    mutate(
      pi = logit_inverse(param$intercept_pi + param$age_pi*age_inconsistent + param$educ_pi*educ_inconsistent + param$age_educ_pi*educ_inconsistent*age_inconsistent),
      mu = param$theta_0/(param$theta_0 + param$theta_1)
    ) %>% 
    mutate(
      p1_star = if_else(y1_star == 1, mu, 1 - mu),
...
calc_lli_derivatives_3waves_ar1_misclassifiction_symptoms_age_educ <- function(param_transformed) {
  ...
  df_grad <- df_probs_temp %>% 
    ...
    mutate(
      ...
      joint_d_age_pi = joint_d_pi*age_inconsistent,
      joint_d_educ_pi = joint_d_pi*educ_inconsistent,
      joint_d_age_educ_pi = joint_d_pi*age_inconsistent*educ_inconsistent
    )
  ...
  df_gi <- df_estimate %>% 
    ...
    mutate(
      ...
      lgi_age_pi = weight*joint_d_age_pi/joint_p,
      lgi_educ_pi = weight*joint_d_educ_pi/joint_p,
      lgi_age_educ_pi = weight*joint_d_age_educ_pi/joint_p,
    ) %>%
    select(lgi_theta_0, lgi_theta_1, lgi_intercept_pi, lgi_age_pi, lgi_educ_pi, lgi_age_educ_pi)
}

calc_mle_3waves_ar1_misclassifiction_symptoms_age_educ <- function(param_transformed) {
  sum(calc_lli_3waves_ar1_misclassifiction_symptoms_age_educ(param_transformed))
}

calc_mle_derivatives_3waves_ar1_misclassifiction_symptoms_age_educ <- function(param_transformed) {
  colSums(calc_lli_derivatives_3waves_ar1_misclassifiction_symptoms_age_educ(param_transformed))
}
