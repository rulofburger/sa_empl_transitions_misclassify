logit_transform <- function(param0) {
  if(include_mu) param0$mu <- param0$theta_2/(param0$theta_1 + param0$theta_2)
...
calc_lli <- function(param_transformed, gradient = T) {
  
  param <- logit_inverse(param_transformed)
  if(!include_mu) param$mu <- param$theta_2/(param$theta_1 + param$theta_2)
    ...
    if(include_mu) {
      df_grads <- df_probs_temp %>% 
        ...
        mutate(
          ...
          d_probs_theta_1 = case_when(
            theta_1_help0 == 2                      ~ joint_p*(-2/(1 - param$theta_1))                  *(1 - param$theta_1)*param$theta_1,
            theta_1_help0 == 1 & theta_1_help1 == 1 ~ joint_p*(-1/(1 - param$theta_1) + 1/param$theta_1)*(1 - param$theta_1)*param$theta_1,
            theta_1_help0 == 1 & theta_1_help1 == 0 ~ joint_p*(-1/(1 - param$theta_1))                  *(1 - param$theta_1)*param$theta_1,
            theta_1_help1 == 1 & theta_1_help0 == 0 ~ joint_p*(1/(param$theta_1))                       *(1 - param$theta_1)*param$theta_1,
            theta_1_help1 == 0 & theta_1_help0 == 0 ~ 0                                                 *(1 - param$theta_1)*param$theta_1
          ),
          d_probs_theta_2 = case_when(
            theta_2_help0 == 2                      ~ joint_p*(-2/(1 - param$theta_2))                  *(1 - param$theta_2)*param$theta_2,
            theta_2_help0 == 1 & theta_2_help1 == 1 ~ joint_p*(-1/(1 - param$theta_2) + 1/param$theta_2)*(1 - param$theta_2)*param$theta_2,
            theta_2_help0 == 1 & theta_2_help1 == 0 ~ joint_p*(-1/(1 - param$theta_2))                  *(1 - param$theta_2)*param$theta_2,
            theta_2_help1 == 1 & theta_2_help0 == 0 ~ joint_p*(1/(param$theta_2))                       *(1 - param$theta_2)*param$theta_2,
            theta_2_help1 == 0 & theta_2_help0 == 0 ~ 0                                                 *(1 - param$theta_2)*param$theta_2
          ),
          d_probs_mu = case_when(
            y1 == 0 ~ joint_p*(-1/(1 - param$mu))*(1 - param$mu)*param$mu,
            y1 == 1 ~ joint_p*(1/param$mu)*(1 - param$mu)*param$mu
          )
        ) %>% 
        group_by(y1, y2, y3) %>% 
        summarise(
      ...
      df_lli <- df_estimate %>% 
        ...
        mutate(
          ...
          lgi_mu = d_probs_mu/joint_p
        ) %>% 
        select(lli, lgi_theta_1, lgi_theta_2, lgi_pi, lgi_mu)
...
calc_ll <- function(param_transformed) {
  ll <- sum(calc_lli(param_transformed, gradient = F))
  return(ll)
}

calc_lg <- function(param_transformed) {
  lg <- calc_lli(param_transformed, gradient = T) %>% select(-lli) %>% colSums
  return(lg)
}
