...
Fact (3) is the empirical foundation for this tenure-extension's main claim: tenure
data corroborates employment-state Markov violations in identifying misclassification.
The base model fails to exploit this corroboration
...
value (a point mass), while the \emph{contaminated branch}
...
(observed); under \(m_{a}=1\), \(T_g\) is a free latent integrated out.

Within the spell, the joint distribution of observed tenures
\((g_{a}, g_{a+1}, \dots, g_{b})\) factorises across the contamination
indicators \((m_{a},\dots,m_{b})\). A wave with \(s_{t}=0\) does not
...
\(s_{t} = 1\) (tenure observed), and let \(t_1 < t_2 < \cdots < t_K\) be
...
m_t = 0\}\) for the set of clean waves, and
...
f\!\big(\{g_t\} \mid \mathbf{m}, h\big),
\label{eq:spell_emission}
\end{equation}
where \(f(\{g_t\} \mid \mathbf{m}, h)\) is the conditional density of the
observed tenures given the contamination pattern, obtained by
...
\item If at least one wave is clean
  ...
  \!\mathbb{1}\!\big\{g_t = g_{t_c} + (t - t_c)\Delta\big\}
...
When \(t_1 > a\) (the spell start is unobserved), the clean and
contaminated branches differ by a shift of \((t_1 - a)\Delta\), and
\(\varepsilon\) remains active.

\paragraph{Pairs (\(K = 2\)): the identification-critical case.}
For state pattern \(s = (1,0,1)\) under \(h = (1,1,1)\): spell \([1,3]\),
\(\mathcal{O} = \{1,3\}\), \(a = 1\), \(K = 2\). The four contamination
patterns yield
\begin{align}
\ell^{\mathrm{spell}}_g \;=\;\;
  & (1-\varepsilon)^2\,\lambda_g\, e^{-\lambda_g g_1}\,
    \mathbb{1}\{g_3 = g_1 + 2\Delta\}
    && (m_1{=}0,\, m_3{=}0) \notag \\
...
no point mass fires and the Markov prior governs the assignment.
...
For a spell with \(K \ge 2\), the posterior probability that at least one
wave is contaminated is
\begin{equation}
\tau_h^{\mathrm{spell}} \;=\;
...
\label{eq:tau_spell}
\end{equation}
where \(f_{\mathbf{0}}\) is the all-clean density (the first line of
\eqref{eq:spell_emission_k2} in the \(K=2\) case). For exactly
clock-consistent observations, \(\tau \to 0\); for inconsistent
observations, \(\tau \to 1\).
